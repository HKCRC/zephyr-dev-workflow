...
| `.\dev.ps1 flash -target none` | 无 bootloader 普通 Zephyr app 烧录。 |
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `-target <target>` | `app` | `none`：普通 Zephyr app，无 bootloader；`app`：只编译已配置的 MCUboot app 子工程；`all`：sysbuild 编译 bootloader + app。 |
| `-board <board>` | `Base.Board` | Zephyr board 名。 |
| `-extra_conf <path>` | 空 | 额外 Kconfig 配置文件。 |
| `-pristine` | 关闭 | `none` 和 `all` 目标可用，用于重新配置；`app` 子工程构建不支持。 |
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `-target <target>` | `app` | `none`：烧普通 app `zephyr.hex`；`app`：烧 MCUboot app；`all`：烧 bootloader + app；`bootloader`：只烧 bootloader；`west`：直接走 west runner。 |
