# zephyr-dev-workflow 迁移与使用指�?
本文说明如何把一个已�?Zephyr 项目迁移�?`zephyr-dev-workflow`，以及迁移后新人如何编译、烧录和 OTA�?
`zephyr-dev-workflow` 是一个可复用 Git submodule，负责提供统一�?PowerShell 开发入口。业务项目只保留自己的配置和一个很薄的入口脚本，不再在每个项目里维护重复的 `build.ps1`、`flash.ps1`、`ota.ps1` 逻辑�?
## 目标结构
...
对外命令和参数统一使用小写加下划线，例�?`image_confirm`、`-dry_run`、`-extra_conf`。旧的根目录脚本和旧参数写法不再维护�?
## 迁移前准�?
| 项目 | 要求 |
| --- | --- |
| 操作系统 | Windows + PowerShell�?|
| Zephyr 构建 | 已经可以�?`west build` 构建�?|
| Python/west | `python -m west` 可用�?|
| Zephyr SDK | 已安装，并能通过项目配置路径找到�?|
| Git | 支持 submodule�?|
| 烧录工具 | 如果使用 STM32，建议安�?STM32CubeProgrammer，并确保 `STM32_Programmer_CLI` 可执行�?|
| OTA 工具 | 如果使用 OTA，需要安�?`mcumgr` 并确保命令可执行�?|
...
提交时需要把 `.gitmodules` �?submodule 指针一起提交：
...
| 自动注入配置 | 默认把项目根目录�?`project_config.json` 传给共享工具�?|
| 转发命令 | �?`build`、`flash`、`ota` 等命令转发给共享工具�?|

## 第三步：新增 project_config.json

在业务项目根目录新增 `project_config.json`。下面是一�?STM32 + MCUboot + mcumgr OTA 项目的示例：
...
| `Build.BuildDir` | 构建目录，支�?`{Board}` 占位符�?|
| `Flash.FlashRunner` | `west flash` 使用�?runner�?|
...
本工具不再保留旧根目录脚本的兼容入口。外�?CI 或生产脚本如果还在调用旧入口，应同步迁移�?`dev.ps1`�?
## 第五步：验证迁移
...
`-target all` 会通过 STM32CubeProgrammer 先烧�?MCUboot，再烧录 confirmed 应用镜像�?
### 只烧�?MCUboot
...
该命令通过 `mcumgr reset` 触发设备重启。如�?OTA 后启动到 test 镜像，但不执�?`image_confirm`，再次重启后 MCUboot 会按未确认镜像处理，通常回滚到旧固件�?
### 查询镜像状�?
```powershell
.\dev.ps1 image_list
```
...
| 检查项 | 通过标准 |
| --- | --- |
| submodule 已添�?| `.gitmodules` 中有 `tool/zephyr-dev-workflow`�?|
...
| 提示缺少 `Board`、`ZephyrBase`、`BuildDir` | 检�?`project_config.json` 是否存在，或者命令是否传�?`-config`�?|
| 修改 DTS 后构建没有变�?| 执行 `.\dev.ps1 build -target all -pristine`�?|
| `flash` 找不�?hex | 先执�?`.\dev.ps1 build`，确�?`Flash.BootloaderHexPath` �?`Flash.AppConfirmedHexPath` 正确�?|
| `ota` 找不�?signed bin | 确认 sysbuild/MCUboot 已启用，�?`Ota.AppSignedBinPath` 与实际构建产物一致�?|
| `mcumgr` 无法连接 | 检查设�?IP、端口、防火墙、网络连通性和固件 mcumgr 服务�?|
| OTA 后回�?| 新固件验证通过后执�?`.\dev.ps1 image_confirm`�?|

## 推荐给新人看的顺�?
1. 先读本文，理解项目如何接�?`zephyr-dev-workflow`�?2. 再读 `doc/command-table.md`，查询每个命令和参数�?3. 最后看业务项目自己�?README，确认板卡、串口、网络、烧录器等项目特定信息�?